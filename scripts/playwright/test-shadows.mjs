/**
 * E2E Tests for Mandelbulb Shadow System
...
async function runTests() {
  ...
  console.log('\n=== E2E Tests for Mandelbulb Shadows ===\n')

  // Navigate to app with mandelbulb object type
  console.log('Loading application with mandelbulb...')
  await page.goto(`${BASE_URL}?t=mandelbulb`, { waitUntil: 'networkidle', timeout: 30000 })
  await page.waitForTimeout(1500) // Wait for React to render and WebGL to initialize

  console.log('\n[Shadow Controls Visibility Tests]')

  await test('App loads with mandelbulb without errors', async () => {
    assertNoErrors()
  ...
  await test('Can load shadow settings from URL', async () => {
    ...
    const urlWithShadows = `${BASE_URL}?t=mandelbulb&se=1&sq=2&ss=1.50&sa=1`
  ...
  console.log('\n[Non-Mandelbulb Object Tests]')

  await test('Shadow controls hidden for non-mandelbulb objects', async () => {
    errors.length = 0
    // Switch to hypercube
    ...
    // Shadow toggle should not be visible for non-mandelbulb
    const shadowToggle = page.locator('[data-testid="shadow-enabled-toggle"]')
    const count = await shadowToggle.count()
    console.log(`    Shadow toggle count (hypercube): ${count}`)
    // Note: May still be 0 even for mandelbulb if no lights are enabled
    assertNoErrors()
  })
