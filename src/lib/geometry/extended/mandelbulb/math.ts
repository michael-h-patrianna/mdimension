/**
 * Core Mathematical Functions for Mandelbulb Generation
 *
 ...
 *
 * @see docs/research/mandelbulb-guide.md for the mandelbulb algorithm
 */
 ...
 */
export function mandelbulb3DStep(z: VectorND, c: VectorND, power: number = 8): VectorND {
  const x = z[0] ?? 0
  const y = z[1] ?? 0
  const zCoord = z[2] ?? 0
 ...
 * @returns Next iterate z_{n+1}
 */
export function mandelbulbStep(z: VectorND, c: VectorND, power: number = 8): VectorND {
  const d = z.length
  ...
  if (d === 3) {
    return mandelbulb3DStep(z, c, power)
  }
...
/**
 * Compute escape time for point c in n-dimensional Mandelbulb
 *
 ...
 *
 * Following standard Mandelbulb convention: step first, then check escape.
...
export function mandelbulbEscapeTime(
  ...
  // Dimension 2: Optimized complex Mandelbulb (no array allocation)
  if (d === 2) {
    let zx = 0
    let zy = 0
    const cx = c[0] ?? 0
