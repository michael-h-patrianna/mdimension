...
vec3 getColorByAlgorithm(float t, vec3 normal, vec3 baseHSL, vec3 position) {
  ...
  if (uColorAlgorithm == 0) {
    ...
    // Normalize by bounding radius (BOUNDING_RADIUS = 2.0)
    float radialT = clamp(length(position) / BOUNDING_RADIUS, 0.0, 1.0);
...
void main() {
    ...
    // Pass normal and position for algorithms that use surface orientation or distance
