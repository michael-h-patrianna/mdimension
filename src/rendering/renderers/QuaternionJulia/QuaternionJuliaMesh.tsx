...
const QuaternionJuliaMesh = () => {
  ...
  useFrame((state) => {
    ...
    u.uSimpleAlpha.value = opacity.simpleAlpha
    ...
    u.uSampleQuality.value = getEffectiveSampleQuality(
      SAMPLE_QUALITY_TO_INT[opacity.sampleQuality] ?? 1,
      fastModeRef.current
    )
    u.uVolumetricReduceOnAnim.value = opacity.volumetricReduceOnAnim

    // Update shadow settings
    u.uShadowEnabled.value = lightStore.shadowEnabled
    u.uShadowQuality.value = getEffectiveShadowQuality(
      SHADOW_QUALITY_TO_INT[lightStore.shadowQuality] ?? 1,
      fastModeRef.current
    )
