/**
 * TSL Hydrogen ND 11D - Hydrogen orbital + 8 HO dimensions
 *
 * Wavefunction: ψ = R_nl(r_11D) × Y_lm(θ,φ) × ∏_{i=3}^{10} φ_{ni}(xi)
 *
 * This is the maximum supported dimension.
 *
 * @module rendering/tsl/raymarching/schroedinger/hydrogenND/hydrogenND11d
 */
...
/**
 * Create the 11D Hydrogen ND wavefunction evaluator.
 */
export function createHydrogenNDPsi11D(uniforms: HydrogenNDUniforms) {
  return Fn(([x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, t]: [Node, Node, Node, Node, Node, Node, Node, Node, Node, Node, Node, Node]) => {
    // Extract omegas from vec4 uniforms
    ...
    const omega4 = uniforms.uExtraDimOmega[1]!.x
    const omega5 = uniforms.uExtraDimOmega[1]!.y
    const omega6 = uniforms.uExtraDimOmega[1]!.z
    const omega7 = uniforms.uExtraDimOmega[1]!.w
    ...
    const earlyExit1 = extraDimEarlyExitCheck(x3, omega0)
      ...
      .or(extraDimEarlyExitCheck(x7, omega4))
      .or(extraDimEarlyExitCheck(x8, omega5))
      .or(extraDimEarlyExitCheck(x9, omega6))
      .or(extraDimEarlyExitCheck(x10, omega7))

    // Compute 11D radius
    const r11D = sqrt(
      x0.mul(x0).add(x1.mul(x1)).add(x2.mul(x2))
        .add(x3.mul(x3)).add(x4.mul(x4)).add(x5.mul(x5))
        .add(x6.mul(x6)).add(x7.mul(x7)).add(x8.mul(x8))
        .add(x9.mul(x9)).add(x10.mul(x10))
    )
    ...
    const n4 = uniforms.uExtraDimN[1]!.x
    const n5 = uniforms.uExtraDimN[1]!.y
    const n6 = uniforms.uExtraDimN[1]!.z
    const n7 = uniforms.uExtraDimN[1]!.w
    ...
    const ef4 = extraDimFactor(n4, omega4, x7)
    const ef5 = extraDimFactor(n5, omega5, x8)
    const ef6 = extraDimFactor(n6, omega6, x9)
    const ef7 = extraDimFactor(n7, omega7, x10)

    // Combine
    const psiReal = R.mul(Y).mul(ef0).mul(ef1).mul(ef2).mul(ef3).mul(ef4).mul(ef5).mul(ef6).mul(ef7)
