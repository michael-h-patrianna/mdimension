/**
 * TSL Hydrogen ND 6D - Hydrogen orbital + 3 HO dimensions
 *
 * Wavefunction: ψ = R_nl(r_6D) × Y_lm(θ,φ) × φ_n3(x3) × φ_n4(x4) × φ_n5(x5)
 *
 * @module rendering/tsl/raymarching/schroedinger/hydrogenND/hydrogenND6d
 */
...
/**
 * Create the 6D Hydrogen ND wavefunction evaluator.
 */
export function createHydrogenNDPsi6D(uniforms: HydrogenNDUniforms) {
  return Fn(([x0, x1, x2, x3, x4, x5, t]: [Node, Node, Node, Node, Node, Node, Node]) => {
    ...
    // Combine
    const psiReal = R.mul(Y).mul(ef0).mul(ef1).mul(ef2)
