/**
 * TSL Hydrogen ND 9D - Hydrogen orbital + 6 HO dimensions
 *
 * Wavefunction: ψ = R_nl(r_9D) × Y_lm(θ,φ) × ∏_{i=3}^{8} φ_{ni}(xi)
 *
 * @module rendering/tsl/raymarching/schroedinger/hydrogenND/hydrogenND9d
 */
...
/**
 * Create the 9D Hydrogen ND wavefunction evaluator.
 */
export function createHydrogenNDPsi9D(uniforms: HydrogenNDUniforms) {
  return Fn(([x0, x1, x2, x3, x4, x5, x6, x7, x8, t]: [Node, Node, Node, Node, Node, Node, Node, Node, Node, Node]) => {
    ...
    // Combine
    const psiReal = R.mul(Y).mul(ef0).mul(ef1).mul(ef2).mul(ef3).mul(ef4).mul(ef5)
