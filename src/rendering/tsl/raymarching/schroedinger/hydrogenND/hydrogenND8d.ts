/**
 * TSL Hydrogen ND 8D - Hydrogen orbital + 5 HO dimensions
 *
 * Wavefunction: ψ = R_nl(r_8D) × Y_lm(θ,φ) × ∏_{i=3}^{7} φ_{ni}(xi)
 *
 * @module rendering/tsl/raymarching/schroedinger/hydrogenND/hydrogenND8d
 */
...
/**
 * Create the 8D Hydrogen ND wavefunction evaluator.
 */
export function createHydrogenNDPsi8D(uniforms: HydrogenNDUniforms) {
  return Fn(([x0, x1, x2, x3, x4, x5, x6, x7, t]: [Node, Node, Node, Node, Node, Node, Node, Node, Node]) => {
    ...
    // Combine
    const psiReal = R.mul(Y).mul(ef0).mul(ef1).mul(ef2).mul(ef3).mul(ef4)
