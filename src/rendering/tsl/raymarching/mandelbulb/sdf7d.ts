/**
 * TSL 7D Mandelbulb SDF
 *
 * 7D Hyperbulb signed distance function with orbit traps.
 * Uses 6 hyperspherical angles.
 *
 * @module rendering/tsl/raymarching/mandelbulb/sdf7d
 */
...
export const createMandelbulb7DSDF = (uniforms: Mandelbulb5D8DUniforms) => {
  return Fn(([pos]: [Vec3Node]) => {
    ...
    const zx = float(cx).toVar('zx'), zy = float(cy).toVar('zy'), zz = float(cz).toVar('zz')
    const z3 = float(c3).toVar('z3'), z4 = float(c4).toVar('z4'), z5 = float(c5).toVar('z5'), z6 = float(c6).toVar('z6')

    const dr = float(1).toVar('dr'), r = float(0).toVar('r')
    const minP = float(1000).toVar('minP'), minASq = float(1000000).toVar('minASq'), minS = float(1000).toVar('minS')
    ...
    Loop(MAX_ITER_HQ, ({ i }) => {
      ...
      r.assign(sqrt(zxzy_sq.add(zz.mul(zz)).add(z3.mul(z3)).add(z4.mul(z4)).add(z5.mul(z5)).add(z6.mul(z6))))
      ...
      const rp = powResult.x.toVar('rp'), rpMinus1 = powResult.y.toVar('rpMinus1')
      dr.assign(rpMinus1.mul(pwr).mul(dr).add(1))

      // 7D: 6 angles
      const t0 = acos(clamp(zz.div(max(r, float(EPS))), float(-1), float(1))).toVar('t0')
      const r1 = sqrt(zxzy_sq.add(z3.mul(z3)).add(z4.mul(z4)).add(z5.mul(z5)).add(z6.mul(z6))).toVar('r1')
      const t1 = r1.greaterThan(EPS).select(acos(clamp(zx.div(max(r1, float(EPS))), float(-1), float(1))), float(0)).toVar('t1')
      const r2 = sqrt(zy.mul(zy).add(z3.mul(z3)).add(z4.mul(z4)).add(z5.mul(z5)).add(z6.mul(z6))).toVar('r2')
      const t2 = r2.greaterThan(EPS).select(acos(clamp(zy.div(max(r2, float(EPS))), float(-1), float(1))), float(0)).toVar('t2')
      const r3 = sqrt(z3.mul(z3).add(z4.mul(z4)).add(z5.mul(z5)).add(z6.mul(z6))).toVar('r3')
      const t3 = r3.greaterThan(EPS).select(acos(clamp(z3.div(max(r3, float(EPS))), float(-1), float(1))), float(0)).toVar('t3')
      const r4 = sqrt(z4.mul(z4).add(z5.mul(z5)).add(z6.mul(z6))).toVar('r4')
      const t4 = r4.greaterThan(EPS).select(acos(clamp(z4.div(max(r4, float(EPS))), float(-1), float(1))), float(0)).toVar('t4')
      const t5 = atan(z6, z5).toVar('t5')
      ...
      const p0 = rp, p1 = p0.mul(s0), p2 = p1.mul(s1), p3 = p2.mul(s2), p4 = p3.mul(s3), p5 = p4.mul(s4)

      zz.assign(p0.mul(c0).add(cz))
      zx.assign(p1.mul(c1).add(cx))
      zy.assign(p2.mul(c2).add(cy))
      z3.assign(p3.mul(c3_).add(c3))
      z4.assign(p4.mul(c4_).add(c4))
      z5.assign(p5.mul(c5_).add(c5))
      z6.assign(p5.mul(s5).add(c6))
      escIt.assign(i)
    })
...
export const createMandelbulb7DSDFSimple = (uniforms: Mandelbulb5D8DUniforms) => {
  return Fn(([pos]: [Vec3Node]) => {
    const bx0 = uniforms.uBasisX0, by0 = uniforms.uBasisY0, bz0 = uniforms.uBasisZ0, o0 = uniforms.uOrigin0
    const bx1 = uniforms.uBasisX1, by1 = uniforms.uBasisY1, bz1 = uniforms.uBasisZ1, o1 = uniforms.uOrigin1
    ...
    const z3 = float(c3).toVar(), z4 = float(c4).toVar(), z5 = float(c5).toVar(), z6 = float(c6).toVar()
    ...
    Loop(MAX_ITER_HQ, ({ i }) => {
      If(i.greaterThanEqual(maxIt), () => Break())
      r.assign(sqrt(zx.mul(zx).add(zy.mul(zy)).add(zz.mul(zz)).add(z3.mul(z3)).add(z4.mul(z4)).add(z5.mul(z5)).add(z6.mul(z6))))
      ...
      z3.assign(p3.mul(c3_).add(c3)); z4.assign(p4.mul(c4_).add(c4)); z5.assign(p5.mul(c5_).add(c5)); z6.assign(p5.mul(s5).add(c6))
    })
