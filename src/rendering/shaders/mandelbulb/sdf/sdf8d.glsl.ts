...
float sdf8D(vec3 pos, float pwr, float bail, int maxIt, out float trap) {
    ...
    for(int i=0;i<MAX_ITER_HQ;i++){
        ...
        float tail=r;
        for(int k=0;k<6;k++){
            t[k]=fast_acos(clamp(z[k] / max(tail, EPS),-1.0,1.0));
        ...
        float s0 = fast_sin((t[0]+phaseT)*pwr), c0 = fast_cos((t[0]+phaseT)*pwr);
        float s1 = fast_sin((t[1]+phaseP)*pwr), c1 = fast_cos((t[1]+phaseP)*pwr);
        z[0]=rp*c0+c[0];
        float sp=rp*s0;
        z[1]=sp*c1+c[1];
        sp*=s1;
        for(int k=2;k<6;k++){
            z[k]=sp*fast_cos(t[k]*pwr)+c[k];
            sp*=fast_sin(t[k]*pwr);
        }
        z[6]=sp*fast_cos(t[6]*pwr)+c[6];
        z[7]=sp*fast_sin(t[6]*pwr)+c[7];
        escIt=i;
    }
...
float sdf8D_simple(vec3 pos, float pwr, float bail, int maxIt) {
    ...
    for(int i=0;i<MAX_ITER_HQ;i++){
        ...
        for(int k=0;k<6;k++){t[k]=fast_acos(clamp(z[k] / max(tail, EPS),-1.0,1.0));tail=sqrt(max(tail*tail-z[k]*z[k],EPS));}
        t[6]=atan(z[7],z[6]);
        float rp=pow(r,pwr);
        // Apply phase shifts to first two angles (theta, phi)
        float s0=fast_sin((t[0]+phaseT)*pwr),c0=fast_cos((t[0]+phaseT)*pwr);
        ...
        z[1]=sp*c1+c[1];
        sp*=s1;
        for(int k=2;k<6;k++){z[k]=sp*fast_cos(t[k]*pwr)+c[k];sp*=fast_sin(t[k]*pwr);}
        z[6]=sp*fast_cos(t[6]*pwr)+c[6];z[7]=sp*fast_sin(t[6]*pwr)+c[7];
    }
    return max(0.5*log(max(r,EPS))*r/max(dr,EPS),EPS);
}
`;
