...
describe('ndTransform', () => {
  ...
  describe('matrixToGPUUniforms', () => {
    ...
    it('should produce correct GPU transformation for 5D XV rotation', () => {
      ...
      const rotated4x4_x = mat4Elements[0] * input[0]! + mat4Elements[4] * input[1]! +
                           mat4Elements[8] * input[2]! + mat4Elements[12] * input[3]!;
      ...
      const gpuDepth = gpuData.depthRowSums[0]! * input[0]! +
                       gpuData.depthRowSums[4]! * input[4]!;
