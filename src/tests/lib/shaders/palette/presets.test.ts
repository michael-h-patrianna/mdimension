...
describe('COSINE_PRESETS', () => {
  it('should have all preset keys defined', () => {
    const expectedKeys: PresetKey[] = [
      // Pastels
      'powderBlue', 'dustyRose', 'softLavender', 'palePeach',
      // Desaturated Blues
      'steelBlue', 'stormCloud', 'deepSea', 'slate', 'fog',
      // Desaturated Reds/Pinks
      'crimsonFade', 'driedRose', 'terracotta', 'clay', 'burgundyMist', 'mauve',
      // Earthy/Neutral
      'stone', 'driftwood', 'charcoal', 'espresso',
      // Two-color blends
      'roseSteel', 'dustyTwilight', 'warmFog', 'coolEmber',
      // Experimental
      'electric', 'plasma', 'nebula', 'prism',
      // Wild/Unconventional
      'glitch', 'infrared', 'acidWash', 'voidPulse', 'solarFlare',
      'deepFry', 'ghostwave', 'toxicSpill', 'binaryFade', 'chromaticShift',
    ];
  ...
  it('should have wild presets with unusual frequencies', () => {
    ...
    for (const key of wildPresets) {
      ...
      const hasUnusualFreq =
        coeffs.c[0] > 1 || coeffs.c[1] > 1 || coeffs.c[2] > 1 ||
