...
describe('Projection Operations', () => {
  ...
  describe('projectEdgesToPositions (Buffer API)', () => {
    ...
    it('applies perspective projection with varying depth', () => {
      ...
      expect(positions[2]).toBeCloseTo(0, 10)
      expect(positions[3]).toBeCloseTo(0, 10)
      expect(positions[4]).toBeCloseTo(1, 10)
      expect(positions[5]).toBeCloseTo(0, 10)
    })

    it('handles invalid edge indices with zeros', () => {
      const vertices = [[1, 2, 3]]
    ...
    it('handles 5D vertices correctly', () => {
      const vertices = [
        [1, 1, 1, 0, 0], // effectiveDepth = 0, scale = 1/4
        [2, 2, 2, 1, 1], // effectiveDepth = 2/sqrt(2), scale varies
      ]
      const edges: [number, number][] = [[0, 1]]
      const positions = new Float32Array(6)
      ...
      expect(positions[0]).toBeCloseTo(0.25, 10)
      expect(positions[1]).toBeCloseTo(0.25, 10)
      expect(positions[2]).toBeCloseTo(0.25, 10)

      // Second vertex: effectiveDepth = (1+1)/sqrt(2) ≈ 1.414
      // denominator = 4 - 1.414 ≈ 2.586
      const expectedDepth = 2 / Math.sqrt(2)
      const expectedScale = 1 / (4 - expectedDepth)
      expect(positions[3]).toBeCloseTo(2 * expectedScale, 5)
      expect(positions[4]).toBeCloseTo(2 * expectedScale, 5)
      expect(positions[5]).toBeCloseTo(2 * expectedScale, 5)
    })
  })
})
