/**
 * Tests for n-dimensional Schroedinger set generation
 *
 * Tests cover:
 * - schroedingerStep() iteration function
 * - schroedingerEscapeTime() escape detection
 * - schroedingerSmoothEscapeTime() smooth coloring
 * - generateSampleGrid() grid generation
 * - filterSamples() point filtering
 * - generateSchroedinger() main generator
 * - getSchroedingerStats() statistics
 * - Hyperspherical coordinate functions (toHyperspherical, fromHyperspherical, powMap)
 * - Schroedinger functions (schroedingerStep, schroedingerEscapeTime)
 */
...
describe('schroedingerStep', () => {
  describe('basic behavior', () => {
    it('should return c when z is zero (origin case)', () => {
      const z = [0, 0, 0]
      const c = [0.5, 0.3, 0.1]
      const result = schroedingerStep(z, c)
    ...
    it('should handle vectors on the axis', () => {
      ...
      const c = [0, 0, 0]
      const result = schroedingerStep(z, c, 8)
      expect(result).toHaveLength(3)
  ...
  describe('power parameter', () => {
    it('should produce different results for different powers', () => {
      const z = [0.5, 0.5, 0.5]
      const c = [0, 0, 0]
      const result3 = schroedingerStep(z, c, 3)
      const result8 = schroedingerStep(z, c, 8)
    ...
    it('should default to power 8', () => {
      const z = [0.5, 0.5, 0.5]
      const c = [0.1, 0.1, 0.1]
      const resultDefault = schroedingerStep(z, c)
      const result8 = schroedingerStep(z, c, 8)
  ...
  describe('spherical coordinate transformation', () => {
    it('should preserve radius for unit vector when power=1', () => {
      ...
      const z = [r, 0, 0]
      const c = [0, 0, 0]
      const result = schroedingerStep(z, c, 1)
    ...
    it('should square radius for power=2', () => {
      ...
      const c = [0, 0, 0]
      const result = schroedingerStep(z, c, 2)
      const resultR = Math.sqrt(result[0]! ** 2 + result[1]! ** 2 + result[2]! ** 2)
...
describe('schroedingerEscapeTime', () => {
  describe('bounded points (inside set)', () => {
    ...
    it('should return maxIter for small c values near origin', () => {
      // For 3D Schroedinger, small values near origin remain bounded
  ...
  describe('escaped points (outside set)', () => {
    it('should return low iteration count for points far from set', () => {
      ...
      const result = schroedingerEscapeTime(c, 100, 4.0)
  ...
  describe('escape radius effects', () => {
    it('should escape earlier with smaller radius', () => {
      const c = [1.5, 0, 0]
      const result1 = schroedingerEscapeTime(c, 100, 2.0)
      const result2 = schroedingerEscapeTime(c, 100, 10.0)
...
describe('schroedingerSmoothEscapeTime', () => {
  it('should return maxIter for bounded points', () => {
    const c = [0, 0, 0]
    const maxIter = 100
    const result = schroedingerSmoothEscapeTime(c, maxIter, 4.0)
    expect(result).toBe(maxIter)
  })

  it('should return fractional values for escaped points', () => {
    const c = [0.4, 0.4, 0]
    const result = schroedingerSmoothEscapeTime(c, 100, 4.0)
    if (result >= 0) {
  ...
  it('should produce values close to discrete version', () => {
    const c = [0.5, 0.5, 0]
    const discrete = schroedingerEscapeTime(c, 100, 4.0)
    const smooth = schroedingerSmoothEscapeTime(c, 100, 4.0)
...
describe('filterSamples', () => {
  ...
  it('should keep ALL points in escapeTime mode (fractal from colors)', () => {
    const config = { ...DEFAULT_SCHROEDINGER_CONFIG, colorMode: 'escapeTime' as const, maxIterations }
  ...
  it('should keep ALL points in smoothColoring mode', () => {
    const config = {
      ...DEFAULT_SCHROEDINGER_CONFIG,
      colorMode: 'smoothColoring' as const,
      maxIterations,
    }
    const filtered = filterSamples(baseSamples, config)
    expect(filtered.length).toBe(baseSamples.length)
  ...
  it('should only keep bounded points in interiorOnly mode', () => {
    const config = {
      ...DEFAULT_SCHROEDINGER_CONFIG,
      colorMode: 'interiorOnly' as const,
      maxIterations,
    }
    const filtered = filterSamples(baseSamples, config)
...
describe('generateSchroedinger', () => {
  ...
  describe('basic generation', () => {
    it('should generate NdGeometry with correct properties', () => {
      const geometry = generateSchroedinger(3, config)

      expect(geometry.dimension).toBe(3)
      expect(geometry.type).toBe('schroedinger')
    ...
    it('should include metadata', () => {
      ...
      // 3D is named 'Schroedinger' not 'Schroedinger'
      expect(geometry.metadata?.name).toBe('Schroedinger')
  ...
  describe('dimension support', () => {
    it('should work for 3D', () => {
      const geometry = generateSchroedinger(3, {
        ...config,
        center: [0, 0, 0],
        parameterValues: [],
      })
    ...
    it('should work for 4D', () => {
      const geometry = generateSchroedinger(4, {
        ...config,
        center: [0, 0, 0, 0],
        parameterValues: [0],
      })
    ...
    it('should work for 7D', () => {
      const geometry = generateSchroedinger(7, {
        ...config,
    ...
    it('should throw for dimension < 3', () => {
      expect(() => generateSchroedinger(1, config)).toThrow()
      expect(() => generateSchroedinger(2, config)).toThrow()
    })
  })

  describe('resolution effects', () => {
    it('should produce more points with higher resolution', () => {
      const lowRes = generateSchroedinger(3, { ...config, resolution: 8 })
      const highRes = generateSchroedinger(3, { ...config, resolution: 16 })
...
describe('getSchroedingerStats', () => {
  ...
  it('should compute total count', () => {
    const stats = getSchroedingerStats(samples, maxIter)
  ...
  it('should handle empty samples', () => {
    const stats = getSchroedingerStats([], maxIter)
  ...
  it('should handle all bounded samples', () => {
    ...
    const stats = getSchroedingerStats(allBounded, maxIter)
    expect(stats.bounded).toBe(2)
    expect(stats.escaped).toBe(0)
...
// =============================================================================
// Hyperspherical Coordinate Tests (for 4D-11D Schroedinger)
// =============================================================================
...
describe('schroedingerHyperbulbEscapeTime', () => {
  describe('bounded points', () => {
    it('should return maxIter for origin', () => {
      const c = [0, 0, 0, 0]
      const maxIter = 60
      const result = schroedingerHyperbulbEscapeTime(c, 8, maxIter, 8)
      expect(result).toBe(maxIter)
    })
  ...
  describe('escaped points', () => {
    it('should escape quickly for points far from origin', () => {
      const c = [3, 0, 0, 0]
      const result = schroedingerHyperbulbEscapeTime(c, 8, 60, 8)
      expect(result).toBeGreaterThanOrEqual(0)
      expect(result).toBeLessThan(10)
    })
  ...
  describe('dimension support', () => {
    it('should work for dimensions 4-11', () => {
      for (const D of [4, 5, 6, 7, 8, 9, 10, 11]) {
        ...
        cFar[0] = 3
        const resultFar = schroedingerHyperbulbEscapeTime(cFar, 8, 60, 8)
  ...
  describe('bailout effects', () => {
    it('should escape earlier with smaller bailout', () => {
      const c = [1.5, 0, 0, 0]
      const result1 = schroedingerHyperbulbEscapeTime(c, 8, 60, 2)
      const result2 = schroedingerHyperbulbEscapeTime(c, 8, 60, 16)
...
describe('schroedingerHyperbulbSmoothEscapeTime', () => {
  it('should return maxIter for bounded points', () => {
    const c = [0, 0, 0, 0]
    const maxIter = 60
    const result = schroedingerHyperbulbSmoothEscapeTime(c, 8, maxIter, 8)
    expect(result).toBe(maxIter)
  })

  it('should return fractional values for escaped points', () => {
    const c = [0.5, 0.5, 0.5, 0.5]
    const result = schroedingerHyperbulbSmoothEscapeTime(c, 8, 60, 8)
    if (result < 60) {
  ...
  it('should produce values close to discrete version', () => {
    ...
    const discrete = schroedingerHyperbulbEscapeTime(c, 8, 60, 8)
    const smooth = schroedingerHyperbulbSmoothEscapeTime(c, 8, 60, 8)
...
describe('schroedingerStep with Schroedinger', () => {
  it('should use schroedingerStep for dimension >= 4', () => {
    // Test that schroedingerStep delegates to schroedingerStep for D >= 4
    const z4 = [0.5, 0.5, 0.5, 0.5]
    const c4 = [0.1, 0.1, 0.1, 0.1]
    const result4 = schroedingerStep(z4, c4, 8)
...
describe('generateSchroedinger with Schroedinger', () => {
  const config = {
    ...DEFAULT_SCHROEDINGER_CONFIG,
  ...
  describe('naming conventions', () => {
    it('should name 3D as Schroedinger', () => {
      const geometry = generateSchroedinger(3, {
        ...config,
        center: [0, 0, 0],
        parameterValues: [],
    ...
    it('should name 4D+ as Schroedinger', () => {
      for (const D of [4, 5, 6, 7, 8, 9, 10, 11]) {
        const geometry = generateSchroedinger(D, {
          ...config,
          center: new Array(D).fill(0),
          parameterValues: new Array(D - 3).fill(0),
        })
        expect(geometry.metadata?.name).toBe(`${D}D Schroedinger`)
      }
    })
  })
  ...
  describe('formula descriptions', () => {
    ...
    it('should describe 4D+ formula with angle count', () => {
      const geometry = generateSchroedinger(6, {
        ...config,
