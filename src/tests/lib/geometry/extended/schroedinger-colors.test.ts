/**
 * Tests for Schroedinger color palette generation
 *
 * Tests cover:
...
import {
  ...
  generatePointColors,
  mapEscapeToColor,
} from '@/lib/geometry/extended/schroedinger/colors';
...
describe('generatePointColors', () => {
  ...
  it('should return empty array for empty escape values', () => {
    const colors = generatePointColors([], DEFAULT_SCHROEDINGER_CONFIG, baseColor);
    expect(colors).toEqual([]);
  });

  it('should generate color for each escape value', () => {
    const escapeValues = [0.1, 0.5, 0.9, 1.0];
    const colors = generatePointColors(escapeValues, DEFAULT_SCHROEDINGER_CONFIG, baseColor);
  ...
  describe('palette effects', () => {
    it('should produce different colors with different palettes', () => {
      ...
      const colors1 = generatePointColors(
        escapeValues,
        { ...DEFAULT_SCHROEDINGER_CONFIG, palette: 'monochrome' },
        baseColor
      );
      const colors2 = generatePointColors(
        escapeValues,
        { ...DEFAULT_SCHROEDINGER_CONFIG, palette: 'complement' },
        baseColor
      );
