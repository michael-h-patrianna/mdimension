/**
 * Tests for n-dimensional Mandelbulb set generation
 *
 * Tests cover:
 * - mandelbulbStep() iteration function
 * - mandelbulbEscapeTime() escape detection
 * - mandelbulbSmoothEscapeTime() smooth coloring
 * - generateSampleGrid() grid generation
 * - filterSamples() point filtering
 * - generateMandelbulb() main generator
 * - getMandelbulbStats() statistics
 * - Hyperspherical coordinate functions (toHyperspherical, fromHyperspherical, powMap)
 * - Mandelbulb functions (mandelbulbStep, mandelbulbEscapeTime)
 */
...
import {
  ...
  powMap,
  toHyperspherical,
} from '@/lib/geometry/extended/mandelbulb'
...
describe('mandelbulbEscapeTime', () => {
  ...
  describe('escaped points (outside set)', () => {
    it('should return low iteration count for points far from set', () => {
      ...
      const result = mandelbulbEscapeTime(c, 100, 4.0)
  ...
  describe('escape radius effects', () => {
    it('should escape earlier with smaller radius', () => {
      const c = [1.5, 0, 0]
      const result1 = mandelbulbEscapeTime(c, 100, 2.0)
      const result2 = mandelbulbEscapeTime(c, 100, 10.0)
...
describe('mandelbulbSmoothEscapeTime', () => {
  it('should return maxIter for bounded points', () => {
    const c = [0, 0, 0]
    const maxIter = 100
    const result = mandelbulbSmoothEscapeTime(c, maxIter, 4.0)
    expect(result).toBe(maxIter)
  })

  it('should return fractional values for escaped points', () => {
    const c = [0.4, 0.4, 0]
    const result = mandelbulbSmoothEscapeTime(c, 100, 4.0)
    if (result >= 0) {
  ...
  it('should produce values close to discrete version', () => {
    const c = [0.5, 0.5, 0]
    const discrete = mandelbulbEscapeTime(c, 100, 4.0)
    const smooth = mandelbulbSmoothEscapeTime(c, 100, 4.0)
...
describe('generateMandelbulb', () => {
  ...
  describe('basic generation', () => {
    it('should generate NdGeometry with correct properties', () => {
      const geometry = generateMandelbulb(3, config)

      expect(geometry.dimension).toBe(3)
      expect(geometry.type).toBe('mandelbulb')
    ...
    it('should include metadata', () => {
      ...
      // 3D is named 'Mandelbulb' not 'Mandelbulb'
  ...
  describe('dimension support', () => {
    it('should work for 3D', () => {
      const geometry = generateMandelbulb(3, {
        ...config,
        center: [0, 0, 0],
        parameterValues: [],
      })
    ...
    it('should work for 4D', () => {
      const geometry = generateMandelbulb(4, {
        ...config,
        center: [0, 0, 0, 0],
        parameterValues: [0],
      })
    ...
    it('should work for 7D', () => {
      const geometry = generateMandelbulb(7, {
        ...config,
    ...
    it('should throw for dimension < 3', () => {
      expect(() => generateMandelbulb(1, config)).toThrow()
      expect(() => generateMandelbulb(2, config)).toThrow()
    })
  })

  describe('resolution effects', () => {
    it('should produce more points with higher resolution', () => {
      const lowRes = generateMandelbulb(3, { ...config, resolution: 8 })
      const highRes = generateMandelbulb(3, { ...config, resolution: 16 })
...
describe('getMandelbulbStats', () => {
  ...
  it('should compute total count', () => {
    const stats = getMandelbulbStats(samples, maxIter)
  ...
  it('should handle empty samples', () => {
    const stats = getMandelbulbStats([], maxIter)
  ...
  it('should handle all bounded samples', () => {
    ...
    const stats = getMandelbulbStats(allBounded, maxIter)
    expect(stats.bounded).toBe(2)
    expect(stats.escaped).toBe(0)
...
// =============================================================================
// Hyperspherical Coordinate Tests (for 4D-11D Mandelbulb)
// =============================================================================
...
describe('mandelbulbStep with Mandelbulb', () => {
  it('should use mandelbulbStep for dimension >= 4', () => {
    // Test that mandelbulbStep delegates to mandelbulbStep for D >= 4
    const z4 = [0.5, 0.5, 0.5, 0.5]
    const c4 = [0.1, 0.1, 0.1, 0.1]
    const result4 = mandelbulbStep(z4, c4, 8)
...
describe('generateMandelbulb with Mandelbulb', () => {
  const config = {
    ...DEFAULT_MANDELBROT_CONFIG,
  ...
  describe('naming conventions', () => {
    ...
    it('should name 4D+ as Mandelbulb', () => {
      for (const D of [4, 5, 6, 7, 8, 9, 10, 11]) {
        const geometry = generateMandelbulb(D, {
          ...config,
          center: new Array(D).fill(0),
          parameterValues: new Array(D - 3).fill(0),
        })
        expect(geometry.metadata?.name).toBe(`${D}D Mandelbulb`)
      }
    })
  })
  ...
  describe('formula descriptions', () => {
    ...
    it('should describe 4D+ formula with angle count', () => {
      const geometry = generateMandelbulb(6, {
        ...config,
