...
describe('Visual Store - Opacity Actions', () => {
  ...
  describe('Initial State', () => {
    it('should have default opacity settings', () => {
      ...
      expect(state.hyperbulbOpacitySettings).toEqual(DEFAULT_OPACITY_SETTINGS);
    });
  ...
  describe('setOpacityMode', () => {
    it('should set opacity mode to simpleAlpha', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.mode).toBe('simpleAlpha');
    });

    it('should set opacity mode to layeredSurfaces', () => {
      useVisualStore.getState().setOpacityMode('layeredSurfaces');
      expect(useVisualStore.getState().hyperbulbOpacitySettings.mode).toBe('layeredSurfaces');
    });

    it('should set opacity mode to volumetricDensity', () => {
      useVisualStore.getState().setOpacityMode('volumetricDensity');
      expect(useVisualStore.getState().hyperbulbOpacitySettings.mode).toBe('volumetricDensity');
    });
    ...
    it('should preserve other settings when changing mode', () => {
      ...
      const settings = useVisualStore.getState().hyperbulbOpacitySettings;
  ...
  describe('setSimpleAlphaOpacity', () => {
    it('should set simple alpha opacity value', () => {
      useVisualStore.getState().setSimpleAlphaOpacity(0.5);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.simpleAlphaOpacity).toBe(0.5);
    });

    it('should clamp value to minimum (0)', () => {
      useVisualStore.getState().setSimpleAlphaOpacity(-0.5);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.simpleAlphaOpacity).toBe(0);
    });

    it('should clamp value to maximum (1)', () => {
      useVisualStore.getState().setSimpleAlphaOpacity(1.5);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.simpleAlphaOpacity).toBe(1);
    });
  });
  ...
  describe('setLayerCount', () => {
    it('should set layer count to 2', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerCount).toBe(2);
    });

    it('should set layer count to 3', () => {
      useVisualStore.getState().setLayerCount(3);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerCount).toBe(3);
    });

    it('should set layer count to 4', () => {
      useVisualStore.getState().setLayerCount(4);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerCount).toBe(4);
    });
  });
  ...
  describe('setLayerOpacity', () => {
    it('should set layer opacity value', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerOpacity).toBe(0.6);
    });

    it('should clamp value to minimum (0.1)', () => {
      useVisualStore.getState().setLayerOpacity(0.05);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerOpacity).toBe(0.1);
    });

    it('should clamp value to maximum (0.9)', () => {
      useVisualStore.getState().setLayerOpacity(0.95);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.layerOpacity).toBe(0.9);
    });
  });
  ...
  describe('setVolumetricDensity', () => {
    it('should set volumetric density value', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.volumetricDensity).toBe(1.5);
    });

    it('should clamp value to minimum (0.1)', () => {
      useVisualStore.getState().setVolumetricDensity(0.05);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.volumetricDensity).toBe(0.1);
    });

    it('should clamp value to maximum (2.0)', () => {
      useVisualStore.getState().setVolumetricDensity(2.5);
      expect(useVisualStore.getState().hyperbulbOpacitySettings.volumetricDensity).toBe(2.0);
    });
  });
  ...
  describe('setSampleQuality', () => {
    it('should set sample quality to low', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.sampleQuality).toBe('low');
    });

    it('should set sample quality to medium', () => {
      useVisualStore.getState().setSampleQuality('medium');
      expect(useVisualStore.getState().hyperbulbOpacitySettings.sampleQuality).toBe('medium');
    });

    it('should set sample quality to high', () => {
      useVisualStore.getState().setSampleQuality('high');
      expect(useVisualStore.getState().hyperbulbOpacitySettings.sampleQuality).toBe('high');
    });
  });
  ...
  describe('setVolumetricAnimationQuality', () => {
    it('should set animation quality to reduce', () => {
      ...
      expect(useVisualStore.getState().hyperbulbOpacitySettings.volumetricAnimationQuality).toBe('reduce');
    });

    it('should set animation quality to full', () => {
      useVisualStore.getState().setVolumetricAnimationQuality('full');
      expect(useVisualStore.getState().hyperbulbOpacitySettings.volumetricAnimationQuality).toBe('full');
    });
  });
