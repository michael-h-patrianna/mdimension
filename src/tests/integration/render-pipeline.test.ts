...
import { createIdentityMatrix, multiplyMatrixVector } from '@/lib/math'
 ...
 * @param axis2
 * @param angle
 * @returns N-dimensional rotation matrix for the specified plane
 */
...
describe('Render Pipeline Integration', () => {
  describe('Full Transform Pipeline', () => {
    it('should generate and transform a 4D hypercube', () => {
      ...
      const rotatedVertices = geometry.vertices.map((v) => multiplyMatrixVector(rotationMatrix, v))
      ...
      const projectedVertices = rotatedVertices.map((v) => projectPerspective(v, 5))
  ...
  describe('Projection Distance Effects', () => {
    it('should affect perspective based on distance', () => {
      ...
      const nearProjection = rotatedVertices.map((v) => projectPerspective(v, 3))
      const farProjection = rotatedVertices.map((v) => projectPerspective(v, 10))
    ...
    objectTypes.forEach((type) => {
      dimensions.forEach((dim) => {
        it(`should render ${type} in ${dim}D`, () => {
          ...
          const rotated = geometry.vertices.map((v) => multiplyMatrixVector(rotation, v))
