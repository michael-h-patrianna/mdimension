...
describe('useIsTablet', () => {
  ...
  it('should return true for tablet screens (md but not lg)', () => {
    window.matchMedia = vi.fn().mockImplementation((query: string) => {
      const isMd = query === BREAKPOINTS.md;
      const isLg = query === BREAKPOINTS.lg;
      return mockMatchMedia(isMd && !isLg);
    });

    const { result } = renderHook(() => useIsTablet());
    // This test is complex due to how the hook works internally
...
describe('BREAKPOINTS', () => {
  ...
  it('should have valid media query strings', () => {
    Object.values(BREAKPOINTS).forEach((query) => {
      expect(query).toMatch(/^\(min-width: \d+px\)$/);
    });
  });
});
