...
# N‑Dimensional Mandelbrot Sets in React + Three.js – Developer Guide

This guide explains, from a **developer perspective**, how to:

1. Understand **Mandelbrot-like sets** beyond 2D.
2. Define a **practical n‑dimensional Mandelbrot function** for dimensions 3–11.
3. **Project** 3D–11D Mandelbrot sets into 3D space.
...
## 1. Quick Recap: Classic 2D Mandelbrot
...
- Iterate:
  \[ z_{n+1} = z_n^2 + c \]
- The **Mandelbrot set** is the set of all \(c\) where the sequence \((z_n)\) stays bounded.
...
export interface NdMandelbrotParams {
...
## 3. A Practical N‑Dimensional Mandelbrot Map

There are many mathematically sophisticated nD generalizations (multicomplex, Clifford algebras, quaternions, etc.).

For implementation, it’s often easier to define a **Mandelbrot‑like** function:
...
export function mandelbrotStepND(z: NdPoint, c: NdPoint): NdPoint {
  const d = z.length;
  if (d < 2) {
...
This is not “the” unique nD Mandelbrot but:

- It **reduces** to classic Mandelbrot if only the first 2 dimensions vary.
...
```ts
export function mandelbrotEscapeTimeND(
  c: NdPoint,
  params: NdMandelbrotParams,
  stepFn = mandelbrotStepND
): number {
...
You now have an **n‑dimensional Mandelbrot evaluator**.
...
- 3D Mandelbrot variant (D = 3): `axes = [0, 1, 2]`, `depthAxis = undefined` (no perspective).
- 4D: `axes = [0, 1, 2]`, `depthAxis = 3` (use the 4th coordinate for perspective).
- 11D: `axes = [0, 1, 2]`, `depthAxis = 3` or any index 3–10.

---

## 5. Building 3D Slices of 3D–11D Mandelbrot Sets
...
2. Pass `c` into `mandelbrotEscapeTimeND`.
3. Use escape time to derive color/intensity.

This gives you a **3D slice** of the D‑dimensional Mandelbrot set.
...
export function sampleSlicePointCloud(
  ...
  return samples.map((world) => {
    ...
    const t = mandelbrotEscapeTimeND(c, params);
...
### 6.2 React Three Fiber component for a D‑dimensional Mandelbrot slice

Here’s an example component that:

- Uses a 4D Mandelbrot (`dimension = 4`).
- Uses axes `[0,1,2]` as world coordinates (x, y, z).
- Treats the 4th coordinate as a parameter controlled by a prop.
- Renders a point cloud.

```tsx
// NdMandelbrotPointCloud.tsx
import React, { useMemo } from 'react';
import { Points } from '@react-three/drei';
import * as THREE from 'three';
import {
  NdMandelbrotParams,
  mandelbrotEscapeTimeND,
  ndZero,
} from './nd-mandelbrot-core';
import { buildPointCloudGeometry } from './pointcloud';

interface NdMandelbrotPointCloudProps {
...
export const NdMandelbrotPointCloud: React.FC<NdMandelbrotPointCloudProps> = ({
  dimension,
  axisIndices,
  paramValues,
  params,
  ...
  const geometry = useMemo(() => {
    ...
    // evaluate Mandelbrot on the slice
...
import { NdMandelbrotPointCloud } from './NdMandelbrotPointCloud';
...
### 7.1 3D Mandelbrot‑like sets (D = 3)
...
### 7.2 4D Mandelbrot‑like sets (D = 4)
...
This lets you **fly through** different slices of a high‑dimensional Mandelbrot space by animating extra coordinates.
...
For a working 3D–11D Mandelbrot visualization:

1. **Core math**
   - [ ] Implement `NdPoint`, `NdMandelbrotParams`.
   - [ ] Implement `ndZero`, `ndAdd`, `ndNorm2`.
   - [ ] Implement `mandelbrotStepND(z, c)`.
   - [ ] Implement `mandelbrotEscapeTimeND(c, params)`.
...
Once this is in place, you can experiment with different **n‑dimensional Mandelbrot maps**, projections, and rendering styles while keeping the same reusable architecture.
