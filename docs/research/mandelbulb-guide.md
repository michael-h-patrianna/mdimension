# Option A (Mandelbulb): “Mandelbulb-style” fractal for 4D–11D (developer-friendly guide)
...
2. Iterate like Mandelbulb: `v = powMap(v) + c`, starting from `v = 0`.
...
- **2D**: Mandelbulb set uses complex numbers.
...
- **`power`**: exponent (float or int), common values:
  ...
  - `2` behaves more “quadratic” but still not the classic 2D Mandelbulb beyond 2D
...
## 2) Core iteration (Mandelbulb-style, but vector-valued)
...
This matches the Mandelbulb pattern: “orbit of 0 under f_c”.
...
```ts
/**
 * Mandelbulb-style power map in D dimensions:
 * 1) convert v -> (r, theta[])
 * 2) r' = r^power
...
## 8) What “4D Mandelbulb” means in your UI

If your UI says “Mandelbulb” for D>2, consider labeling it as:
- **“Mandelbulb (Mandelbulb generalization)”**
or
- **“Mandelbulb-like (hyperspherical power)”**

Because it’s not the canonical Mandelbulb set anymore (that’s uniquely defined in 2D complex numbers).
