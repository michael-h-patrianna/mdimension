# Hyperbulb Opacity Modes

## Overview

This feature adds four distinct opacity/transparency rendering modes for hyperbulb fractals, allowing users to visualize the fractal structure with varying degrees of transparency. Users access these settings through the Faces section > Material tab in the sidebar, reusing the existing opacity slider infrastructure used by polytopes.

---

## User Story 1: Opacity Mode Selection

**User story:** As a user viewing a hyperbulb fractal, I want to select between different opacity rendering modes so that I can visualize the fractal with varying transparency effects.

**Acceptance criteria**
1. User sees an "Opacity Mode" dropdown in the Material tab when viewing a hyperbulb fractal (Mandelbulb 3D+)
...
6. When switching from hyperbulb to a polytope object type, the opacity mode dropdown is hidden
7. When switching back to hyperbulb, the previously selected mode is restored
...
- Given the user is viewing a 4D hyperbulb with "Solid" mode selected
...
- Given the user has selected "Volumetric Density" mode for a hyperbulb
...
**User story:** As a user, I want to view the hyperbulb as a fully opaque solid surface so that I can see the exterior form clearly without any transparency effects.
...
- When the user views the hyperbulb from any angle
...
- Given the user is rotating a 7D hyperbulb in "Solid" mode
...
**User story:** As a user, I want to apply uniform transparency to the hyperbulb surface so that I can see a faded version of the fractal that reveals objects behind it.
...
**User story:** As a user, I want to see multiple transparent layers of the hyperbulb surface so that I can visualize the internal structure and depth of the fractal.
...
**User story:** As a user, I want to render the hyperbulb as a volumetric cloud-like structure so that I can see the fractal's density distribution throughout its volume.
...
- Then the new tab shows the hyperbulb in Simple Alpha mode with opacity 0.4
...
**Feature**: Hyperbulb Opacity Modes
