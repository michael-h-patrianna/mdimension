...
Implementation of the Quaternion Julia fractal type following the Mandelbulb (Mandelbulb) pattern. This plan ensures feature parity with existing raymarched fractals including scale parameter, walls auto-positioning, and proper registration for "Faces" render mode.
 ...
 * Follows the pattern established by mandelbulbSlice and mandelboxSlice.
 */
...
Key sections:
1. Uniforms declaration (matching MandelbulbMesh uniforms)
...
The shader should be ~1500-2000 lines following the `mandelbulb.frag` structure but with:
...
Follow `MandelbulbMesh.tsx` pattern (~700 lines) with these changes:
...
// 3. determineRenderMode function (after line 76, before mandelbulb check)
...
function isRaymarchedFractal(objectType: string, dimension: number): boolean {
  return (
    (objectType === 'mandelbulb' && dimension >= 3) ||
...
Follow `MandelbulbControls.tsx` pattern with these sections:
...
## Key Differences from Mandelbulb

| Aspect | Mandelbulb (Mandelbulb) | Quaternion Julia |
|--------|------------------------|------------------|
