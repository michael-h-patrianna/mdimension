# N-Dimensional Mandelbulb Set Visualization

## Product Overview

An extension to the N-Dimensional Object Visualizer that adds n-dimensional Mandelbulb sets (dimensions 3-11) as a new object type. Users can explore fractal structures in higher dimensions, navigate to interesting coordinates, adjust iteration parameters, and visualize 3D slices of these complex mathematical objects.

**Target Audience:** Fractal enthusiasts, mathematics educators, students, digital artists, and anyone curious about how the Mandelbulb set extends into higher dimensions.

**Core Value Proposition:** Enable exploration of Mandelbulb-like fractals beyond the familiar 2D images, revealing how fractal structures manifest in 3D, 4D, and higher dimensional spaces through interactive 3D projections.

---

## User Story 1: Select Mandelbulb Set as Object Type

**User story:** As a user, I want to select "Mandelbulb Set" as an object type so that I can visualize n-dimensional Mandelbulb fractals instead of geometric polytopes.

**Acceptance criteria**
1. "Mandelbulb Set" appears as a new option in the Object Type selector alongside Hypercube, Simplex, Cross-polytope, and Demihypercube
2. Selecting "Mandelbulb Set" replaces the current polytope with a 3D visualization of the Mandelbulb set
3. Default dimension for Mandelbulb set is 4D (providing the classic 2D complex plane behavior plus two extra dimensions for exploration)
4. Mandelbulb set supports dimensions 3 through 11 (matching the existing dimension selector range for higher dimensions)
5. When Mandelbulb set is selected, a new "Fractal Settings" section appears in the control panel
6. The Mandelbulb visualization renders within 2 seconds on initial selection (using default low-resolution settings)
7. Object type selection persists in URL state for sharing
8. Switching away from Mandelbulb set and back preserves the last-used fractal settings

**Test scenarios**

Scenario 1: Select Mandelbulb set from object type dropdown
- Given the user has a 4D hypercube displayed
- When the user selects "Mandelbulb Set" from the object type selector
- Then a 4D Mandelbulb set visualization appears in the viewport

Scenario 2: Mandelbulb set with dimension change
- Given the user has a 4D Mandelbulb set displayed
- When the user changes dimension to 6D
- Then the visualization updates to show a 6D Mandelbulb set with additional parameter controls

Scenario 3: Switch between object types
- Given the user has customized Mandelbulb fractal settings
- When the user switches to "Hypercube" then back to "Mandelbulb Set"
- Then the previous fractal settings are restored

Scenario 4: URL sharing includes object type
- Given the user has a 5D Mandelbulb set with custom settings
- When the user copies the share URL
- Then loading that URL displays the same 5D Mandelbulb configuration

---

## User Story 2: Configure Iteration Parameters

**User story:** As a user, I want to adjust the Mandelbulb iteration parameters so that I can control the detail level and structure of the fractal visualization.
...
9. Tooltip explains: "Escape Radius is the threshold distance beyond which a point is considered to have escaped. Values near 2.0 give classic Mandelbulb boundaries."

**Test scenarios**

Scenario 1: Increase max iterations
- Given a Mandelbulb set with maxIter=30
...
- Given a Mandelbulb set with escapeRadius=4.0
...
- Given a Mandelbulb set at "Draft" quality
...
- Given a 4D Mandelbulb set at 32x32x32 resolution
...
- Given a Mandelbulb set at resolution 32
...
**User story:** As a user, I want to choose which three dimensions of the n-dimensional Mandelbulb set are displayed as the 3D visualization axes so that I can explore different cross-sections of higher-dimensional fractals.
...
- Given a 5D Mandelbulb set with axes X=0, Y=1, Z=2
...
- Given a 7D Mandelbulb set is selected
...
- Given a 6D Mandelbulb set with axes X=0, Y=1, Z=2
...
**User story:** As a user, I want to adjust the values of non-visualized dimensions using sliders so that I can navigate through different cross-sections of the higher-dimensional Mandelbulb set.
...
- Given a 4D Mandelbulb set with axes X=0, Y=1, Z=2
...
- Given a 5D Mandelbulb set with parameter Dim 4 at 0.0
...
- Given a 6D Mandelbulb with Dim 3=0.3, Dim 4=-0.1, Dim 5=0.7
- When the user copies the share URL
...
**User story:** As a user, I want to navigate to pre-defined interesting coordinates in the Mandelbulb set so that I can quickly explore visually stunning regions without manual searching.
...
- Given a 4D Mandelbulb set at default center
...
- Given a 3D Mandelbulb set is displayed
...
- Then the view jumps to a random point within the Mandelbulb set boundary
...
**User story:** As a user, I want to zoom into and pan around the Mandelbulb set so that I can explore fine details and navigate to specific regions.
...
6. "Fit to View" button resets to show the full Mandelbulb set extent (extent=2.5, centered at origin)
...
- Given a Mandelbulb set with "Classic" palette
...
- Then points inside the Mandelbulb set render as dark purple

---

## User Story 9: Choose Rendering Style

**User story:** As a user, I want to choose between different rendering styles so that I can visualize the Mandelbulb set in ways that best reveal its structure.
...
- Given a Point Cloud rendering of the Mandelbulb set
...
- Given a 5D Mandelbulb set with Dim 3 parameter at 0.0
...
- Given a 7D Mandelbulb set with 4 parameter dimensions
...
## User Story 12: Apply Visual Shaders to Mandelbulb

**User story:** As a user, I want to apply the existing visual shaders to the Mandelbulb visualization so that I can create stunning visual effects.

**Acceptance criteria**
1. All existing shaders work with Mandelbulb visualizations: Wireframe, Neon Glow, Gradient Fill, Surface
...
- Given a Mandelbulb Isosurface rendering
...
Scenario 3: Bloom on Mandelbulb
- Given a Mandelbulb visualization
...
- Given a 4D Mandelbulb with isosurface rendering
- When the user enables "Depth-coded coloring"
- Then parts of the surface at different W-coordinates show different hues

---

## User Story 13: View Mandelbulb Properties

**User story:** As a user, I want to view mathematical properties and information about the current Mandelbulb configuration so that I can understand what I'm visualizing.

**Acceptance criteria**
1. Properties panel shows current Mandelbulb configuration
2. Displayed properties include:
   - Object type: "Mandelbulb Set (nD)"
   - Current dimension
   - Visualization axes
...
4. "About Mandelbulb Sets" link opens educational content explaining the generalization to n dimensions
5. Coordinate display updates in real-time as parameters change

**Test scenarios**

Scenario 1: View 5D Mandelbulb properties
- Given a 5D Mandelbulb set is displayed
- When the user views the properties panel
- Then it shows: "Mandelbulb Set (5D)", visualization axes, parameter values for Dim 3 and 4, etc.
...
- When the Mandelbulb is rendering
...
**Feature**: N-Dimensional Mandelbulb Set Visualization
...
|---|-------|------|-----------|--------------|
| 1 | Select Mandelbulb as Object Type | User | ~2 days | None (core integration) |
...
| 13 | View Mandelbulb Properties | User | ~1 day | Story 1 |
...
### Open Questions
- None - the research guide `docs/research/nd-mandelbulb-threejs-guide.md` provides all necessary mathematical foundations

### Dependencies Between Stories
- Story 1 (Mandelbulb object type) is foundational for all other stories
...
Based on research guide `docs/research/nd-mandelbulb-threejs-guide.md`:
...
- Object type selector: Add "Mandelbulb Set" option
- Dimension selector: Reuse existing control (extend range to 11 if needed)
- Visual shaders: Apply existing shader system to Mandelbulb outputs
- Bloom/post-processing: Works with all render styles
- URL state: Extend serializer to include Mandelbulb-specific parameters
- Properties panel: Extend to show Mandelbulb properties
