# Mandelbulb Raymarching for 4D-11D Dimensions

## Specification Summary

**Feature**: GPU-accelerated raymarching surface rendering for 4D-11D Mandelbulb fractals
...
|---|-------|------|-----------|--------------|
| 1 | Mandelbulb Surface Rendering (4D-11D) | User | ~2 days | None |
...
The application currently supports two rendering modes for Mandelbulb-like fractals:
...
This PRD extends the raymarching capability to dimensions 4D through 11D, rendering "Mandelbulb" fractals - the higher-dimensional generalization of the Mandelbulb using hyperspherical coordinates.
...
## User Story 1: Mandelbulb Surface Rendering (4D-11D)

**User story:** As a user exploring higher-dimensional fractals, I want to view 4D-11D Mandelbulb fractals as solid raymarched surfaces so that I can see smooth, lit geometry instead of point clouds.

**Acceptance criteria**

1. When dimension is 4-11 and "Surface" toggle is ON, the system displays a raymarched Mandelbulb surface
...
4. The raymarched surface shows a 3D slice of the D-dimensional Mandelbulb, where D is the current dimension
...
8. The "Power" setting (2-16) controls the Mandelbulb shape, matching point cloud behavior
...
12. The ground plane (when enabled) positions correctly beneath the raymarched Mandelbulb surface

**Test scenarios**

Scenario 1: Enable surface rendering for 4D Mandelbulb
- Given the user has selected Mandelbulb object type and dimension is set to 4
- When the user turns ON the "Surface" toggle
- Then the system displays a raymarched 4D Mandelbulb surface with smooth shading and the point cloud disappears

Scenario 2: Disable surface rendering returns to point cloud
- Given the user is viewing a raymarched 5D Mandelbulb surface
...
- Given the user is viewing a raymarched 4D Mandelbulb with "Surface" toggle ON
- When the user changes dimension to 7D
- Then the system displays a raymarched 7D Mandelbulb surface (not point cloud)

Scenario 4: Power setting affects surface shape
- Given the user is viewing a raymarched 6D Mandelbulb with power=8
...
- Then the system displays a raymarched 4D Mandelbulb surface (surface mode remains active)

Scenario 6: High escape radius for stability
- Given the user is viewing an 8D Mandelbulb with escape radius set to 4.0
...
**User story:** As a user exploring higher-dimensional Mandelbulbs, I want to adjust slice parameters for dimensions beyond X/Y/Z so that I can explore different cross-sections of the D-dimensional fractal.
...
7. Slice parameter controls appear in the Mandelbulb Controls panel when dimension >= 4 and "Surface" toggle is ON
...
- Given the user is viewing a raymarched 5D Mandelbulb with Dim 3 slice = 0.0
...
- Given the user has modified Dim 3 = 0.8 and Dim 4 = -0.3 for a 6D Mandelbulb
...
- When the Mandelbulb Controls panel is visible
...
- Given the user is viewing a 6D Mandelbulb point cloud with "Surface" toggle OFF
- When the user views the Mandelbulb Controls panel
...
- Given the user has set Dim 3 = 0.5 for a 5D Mandelbulb
...
- Given the user has set Dim 3 = 0.5 for a 4D Mandelbulb
...
**User story:** As a user, I want the raymarching quality to adapt based on dimension so that I maintain acceptable frame rates when viewing higher-dimensional Mandelbulbs.

**Acceptance criteria**

1. The system maintains interactive frame rates (target: 30+ FPS) for raymarched Mandelbulbs across all supported dimensions
...
- Given the user enables surface rendering for a 4D Mandelbulb with default settings
...
- Given the user enables surface rendering for an 11D Mandelbulb with max iterations = 200
...
- Given the user is experiencing slow frame rates with a 9D Mandelbulb at 100 iterations
...
- Given the user switches to dimension 10D with Mandelbulb object type
- When the system initializes the Mandelbulb configuration
- Then max iterations is set to a conservative default (lower than 4D default)

Scenario 5: Window resize affects performance
- Given the user is viewing an 8D Mandelbulb with moderate frame rate
...
**User story:** As a user, I want the raymarched Mandelbulb surface to respect all visual settings (lighting, colors, palettes) so that it matches the appearance of the 3D Mandelbulb and other objects.

**Acceptance criteria**

1. The "Surface Color" setting controls the base color of the raymarched Mandelbulb surface
...
4. The "Light" toggle enables/disables directional lighting on the Mandelbulb surface
...
11. The raymarched Mandelbulb casts/receives shadows on the ground plane (when ground plane is enabled)
12. Surface color changes take effect immediately without re-rendering the entire scene

**Test scenarios**

Scenario 1: Surface color affects Mandelbulb appearance
- Given the user is viewing a raymarched 5D Mandelbulb with surface color set to blue
- When the user changes surface color to red
- Then the Mandelbulb surface immediately displays with red-based coloring

Scenario 2: Palette mode changes color variation
- Given the user is viewing a raymarched 6D Mandelbulb with Monochromatic palette
...
- Given the user is viewing a raymarched 4D Mandelbulb with light enabled
...
- Given the user is viewing a raymarched 7D Mandelbulb with specular intensity = 1.0
...
- Given the user is viewing a raymarched 5D Mandelbulb with light enabled and ambient = 0.3
...
- Given the user is viewing a raymarched 8D Mandelbulb with light enabled
...
The Mandelbulb uses the same iteration formula as the point cloud:
