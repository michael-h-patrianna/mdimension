...
│ ┌─ Preset Manager ────────────────────────┐   │
│ │ [My Presets                      ▼]      │   │
│ │   • Default (Cosine Rainbow)             │   │
│ │   • Fire & Brimstone (saved)             │   │
│ │   • Ocean Depths (saved)                 │   │
│ │   • Purple Dream (saved)                 │   │
│ │   • --- Shared Presets ---               │   │
│ │   • Community: Neon City                 │   │
│ │   • Community: Pastel Waves              │   │
│ │                                          │   │
│ │ 💾 Save Current   📋 Rename   🗑️ Delete  │   │
...
- Dropdown with 7 main categories:
  1. **Solid Color** - No variation, uses base color only
  2. **Cosine Gradients** ★ - Recommended, smooth cyclic palettes
  3. **LCH/Oklab Color Space** - Perceptually uniform colors
  4. **Normal-Based (MatCap)** - Color by surface orientation
  5. **Distance-Field Gradient** - Color by distance/depth
  6. **Multi-Source Mapping** - Advanced per-channel control
  7. **Hybrid/Custom Blend** - Combine multiple algorithms
- Each selection shows/hides different control panels below
- Default: "Cosine Gradients" (recommended)
- Shows availability badges: ★ Recommended, 🌍 Universal, ⚡ Raymarching Only
...
**6. Solid Color Mode** (Conditional)
- Only shown when "Solid Color" algorithm selected
- Uses base color picker only
- No variation controls visible (all disabled)
- Simple message: "Using solid color - no palette variation applied"

**7. Preset Manager** (Always Visible)

**Purpose**: Allow users to save, load, share, and manage complete color configurations.

**Features**:
- **Save Current**: Stores all current settings with user-provided name
- **Rename**: Edit preset name
- **Delete**: Remove saved preset (with confirmation)
- **Export**: Download preset as JSON file for sharing
- **Import**: Load preset from JSON file
- **Reset to Default**: Restore recommended default settings
- **Preset Categories**:
  - User Presets (saved locally)
  - Shared/Community Presets (optional: loaded from remote)
  - Built-in Defaults (factory presets, cannot be deleted)
...
**Storage**:
- User presets: localStorage (key: `mdimension_color_presets`)
- Max presets: 50 per user (prevent storage bloat)
- Auto-save on modification: optional setting

**UI/UX Requirements**:
- Dropdown shows preset name + small preview gradient
- Hovering preset shows tooltip with settings summary
- Applying preset shows confirmation: "Applied preset: [name]"
- Unsaved changes indicator (asterisk next to current name)
- Export creates `preset-name.json` download
- Import validates JSON schema before applying
- Built-in presets marked with 🔒 icon (read-only)

**Validation**:
- Preset names: 1-50 characters, alphanumeric + spaces
- JSON schema validation on import
- Version checking (warn if importing newer version)
- Fallback to defaults if corrupted preset detected

### Algorithm-Specific UI States

#### When "Solid Color" Selected:
```
✓ Base color picker visible
✗ Palette controls hidden
✗ Distribution controls hidden (disabled)
✗ Advanced options hidden
✗ All variation controls hidden
ℹ️ Message: "Using solid color - no palette variation applied"
```
...
interface ColorSettings {
  // Primary algorithm
  algorithm: 'solid' | 'cosine' | 'lch' | 'normal' | 'distance' | 'multiSource' | 'hybrid'

  // Base color (used by all algorithms)
  baseColor: {
    h: number  // 0-360
    s: number  // 0-100
    l: number  // 0-100
  ...
  cosinePalette: {
    preset: 'rainbow' | 'fire' | 'ocean' | 'sunset' | 'forest' | 'candy' | 'neon' | 'pastel' | 'monochrome' | 'custom'
  ...
  // Preset management
  presets: {
    current: string | null     // Currently applied preset name
    userPresets: ColorPreset[] // User-saved presets
...
- `src/components/controls/color/PresetManager.tsx` - Enhanced with export/import
...
- `src/lib/presets/colorPresets.ts` - Built-in preset definitions
