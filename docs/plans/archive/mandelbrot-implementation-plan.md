# N-Dimensional Mandelbrot Set Implementation Plan

**PRD**: `docs/prd/ndimensional-mandelbrot.md`
**Math Guide**: `docs/research/nd-mandelbrot-threejs-guide.md`
...
This plan implements n-dimensional Mandelbrot set visualization (3D-11D) as a new object type in the existing visualizer. The implementation leverages the existing rendering pipeline, shader system, and visual settings wherever possible, only adding Mandelbrot-specific features where necessary.
...
| Feature | Existing System | Mandelbrot Usage |
|---------|----------------|------------------|
...
| Object Type Selector | `ObjectTypeSelector.tsx` | Add "Mandelbrot Set" option |
| Dimension Selector | `DimensionSelector.tsx` | Reuse for 3-11D selection |
| URL State | `state-serializer.ts` | Persist Mandelbrot config |
...
| Zoom/Pan (Extent) Controls | Navigate within Mandelbrot space |
...
│   ├── mandelbrot.ts           # Core generator + iteration algorithm
│   ├── mandelbrot-locations.ts # Predefined interesting coordinates
│   ├── mandelbrot-colors.ts    # Color palettes + escape-time mapping
│   ├── marching-cubes.ts       # Isosurface mesh generation
│   └── mandelbrot-worker.ts    # Web Worker for background computation
│
├── stores/
│   ├── mandelbrotStore.ts      # Main Mandelbrot state
│   └── mandelbrotBookmarkStore.ts # User bookmarks (localStorage)
│
├── components/controls/
│   ├── MandelbrotControls.tsx       # Main settings panel
│   ├── MandelbrotAxisSelector.tsx   # Visualization axes
│   ├── MandelbrotParameterSliders.tsx # Non-viz dimension controls
│   ├── MandelbrotColorControls.tsx  # Color mapping settings
│   ├── MandelbrotLocations.tsx      # Interesting locations dropdown
│   ├── MandelbrotAnimationControls.tsx # Parameter animation
│   ├── MandelbrotBookmarks.tsx      # Save/load bookmarks
│   └── MandelbrotProperties.tsx     # Info panel
│
└── components/canvas/
    └── MandelbrotIsosurfaceRenderer.tsx # Marching cubes mesh renderer
```
...
**Goal**: Basic Mandelbrot point cloud rendering with iteration controls
...
export function isExtendedObjectType(type: string): type is ExtendedObjectType {
  return ['hypersphere', 'root-system', 'clifford-torus', 'mandelbrot'].includes(type);
}
```
...
**File**: `src/lib/geometry/extended/mandelbrot.ts`
```typescript
...
**File**: `src/stores/mandelbrotStore.ts`
```typescript
import { create } from 'zustand';
import { DEFAULT_MANDELBROT_CONFIG, type MandelbrotConfig } from '@/lib/geometry/extended/types';

interface MandelbrotState extends MandelbrotConfig {
  // Actions - Iteration
  setMaxIterations: (value: number) => void;
  setEscapeRadius: (value: number) => void;
  setQualityPreset: (preset: MandelbrotConfig['qualityPreset']) => void;
  ...
  // Actions - Color
  setColorMode: (mode: MandelbrotConfig['colorMode']) => void;
  setPalette: (palette: MandelbrotConfig['palette']) => void;
  setCustomPalette: (palette: MandelbrotConfig['customPalette']) => void;
  setInvertColors: (invert: boolean) => void;
  setInteriorColor: (color: string) => void;
  setPaletteCycles: (cycles: number) => void;

  // Actions - Rendering
  setRenderStyle: (style: MandelbrotConfig['renderStyle']) => void;
...
export const useMandelbrotStore = create<MandelbrotState>((set, get) => ({
...
```typescript
// Add export
export { generateMandelbrot, mandelbrotStep, mandelbrotEscapeTime } from './mandelbrot';

// Add to generateExtendedObject switch:
case 'mandelbrot':
  return generateMandelbrot(dimension, params.mandelbrot);
```
...
```typescript
// Add 'mandelbrot' to VALID_OBJECT_TYPES array
```
...
**File**: `src/tests/lib/geometry/extended/mandelbrot.test.ts`
- Test `mandelbrotStep()` for known inputs
- Test `mandelbrotEscapeTime()` for points inside/outside set
- Test `generateMandelbrot()` returns valid geometry
...
**Goal**: Full navigation through n-dimensional Mandelbrot space

### 2.1 Axis Selector

**File**: `src/components/controls/MandelbrotAxisSelector.tsx`
- Three dropdowns for X, Y, Z axis selection
- Options: "Dim 0 (Re)", "Dim 1 (Im)", "Dim 2", ... "Dim N-1"
- Prevent duplicate selection
- Update mandelbrotStore.visualizationAxes

### 2.2 Parameter Sliders

**File**: `src/components/controls/MandelbrotParameterSliders.tsx`
...
**File**: `src/components/controls/MandelbrotNavigationControls.tsx`
...
- Mouse scroll zoom for Mandelbrot (when object type is mandelbrot)
- Click-drag pan for Mandelbrot
- Update mandelbrotStore.center and extent
...
**File**: `src/lib/geometry/extended/mandelbrot-colors.ts`
```typescript
...
**File**: `src/components/controls/MandelbrotColorControls.tsx`
- Color mode dropdown
...
**File**: `src/components/canvas/MandelbrotIsosurfaceRenderer.tsx`
...
**File**: `src/lib/geometry/extended/mandelbrot-locations.ts`
```typescript
...
**File**: `src/components/controls/MandelbrotLocations.tsx`
...
**File**: `src/components/controls/MandelbrotAnimationControls.tsx`
...
**File**: `src/stores/mandelbrotBookmarkStore.ts`
- Save/load full configuration
- LocalStorage persistence
- Max 50 bookmarks
- Import/export JSON

**File**: `src/components/controls/MandelbrotBookmarks.tsx`
- Save with name
- Load from list
- Delete individual
...
**File**: `src/components/controls/MandelbrotProperties.tsx`
...
**File**: `src/lib/geometry/extended/mandelbrot-worker.ts`
...
**File**: `src/components/controls/MandelbrotPerformanceControls.tsx`
...
| File | Sprint | Purpose |
|------|--------|---------|
| `src/lib/geometry/extended/mandelbrot.ts` | 1 | Core algorithm |
| `src/stores/mandelbrotStore.ts` | 1 | State management |
| `src/components/controls/MandelbrotControls.tsx` | 1 | Main settings |
| `src/tests/lib/geometry/extended/mandelbrot.test.ts` | 1 | Unit tests |
| `src/components/controls/MandelbrotAxisSelector.tsx` | 2 | Axis selection |
| `src/components/controls/MandelbrotParameterSliders.tsx` | 2 | Param controls |
| `src/components/controls/MandelbrotNavigationControls.tsx` | 2 | Zoom/pan |
| `src/lib/geometry/extended/mandelbrot-colors.ts` | 3 | Color palettes |
| `src/components/controls/MandelbrotColorControls.tsx` | 3 | Color UI |
| `src/lib/geometry/extended/marching-cubes.ts` | 3 | Isosurface |
| `src/components/canvas/MandelbrotIsosurfaceRenderer.tsx` | 3 | Isosurface renderer |
| `src/lib/geometry/extended/mandelbrot-locations.ts` | 4 | Preset locations |
| `src/components/controls/MandelbrotLocations.tsx` | 4 | Locations UI |
| `src/components/controls/MandelbrotAnimationControls.tsx` | 4 | Animation |
| `src/stores/mandelbrotBookmarkStore.ts` | 4 | Bookmarks |
| `src/components/controls/MandelbrotBookmarks.tsx` | 4 | Bookmarks UI |
| `src/components/controls/MandelbrotProperties.tsx` | 4 | Properties panel |
| `src/lib/geometry/extended/mandelbrot-worker.ts` | 5 | Web Worker |
| `src/components/controls/MandelbrotPerformanceControls.tsx` | 5 | Perf controls |
...
| File | Changes |
|------|---------|
| `src/lib/geometry/types.ts` | Add 'mandelbrot' type |
| `src/lib/geometry/extended/types.ts` | Add MandelbrotConfig |
...
| `src/components/controls/ObjectSettingsSection.tsx` | Render Mandelbrot controls |
...
### Unit Tests
- `mandelbrot.test.ts` - Algorithm correctness
- `mandelbrot-colors.test.ts` - Color mapping
- `marching-cubes.test.ts` - Mesh generation
- `mandelbrotStore.test.ts` - Store actions

### Component Tests
- `MandelbrotControls.test.tsx`
- `MandelbrotAxisSelector.test.tsx`
- `MandelbrotParameterSliders.test.tsx`
- `MandelbrotColorControls.test.tsx`
...
- All shaders work with Mandelbrot
