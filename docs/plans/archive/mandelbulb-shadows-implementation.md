# Mandelbulb Soft Shadows - Implementation Plan

## Overview

This plan implements soft shadow rendering for mandelbulb fractals with user-configurable quality and softness settings, as specified in `docs/prd/mandelbulb_shadows.md`.
...
#### 2.1 Update Fragment Shader (`mandelbulb.frag`)
...
```tsx
{/* Shadow Controls - Only for Mandelbulb with enabled lights */}
{isMandelbulb && hasEnabledLights && (
  <>
...
- Test toggle visibility when mandelbulb + lights enabled
- Test toggle hidden when no lights
- Test toggle hidden when not mandelbulb
...
| `src/components/canvas/renderers/Mandelbulb/mandelbulb.frag` | Modify | 1, 4, 5 |
| `src/components/canvas/renderers/Mandelbulb/MandelbulbMesh.tsx` | Modify | 1, 5 |
...
|-------|-----|------------|
| 1 | 8 | visualStore, FacesSection, mandelbulb.frag |
| 2 | 8 | visualStore, FacesSection, mandelbulb.frag |
| 3 | 8 | visualStore, FacesSection, mandelbulb.frag |
| 4 | 8 | mandelbulb.frag (light type handling) |
