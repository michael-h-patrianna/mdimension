# Mandelbulb Opacity Modes - Implementation Plan

Based on PRD: `docs/prd/mandelbulb_opacity.md`

## Overview

This plan implements 4 opacity rendering modes for mandelbulb fractals:
...
- State property: `mandelbulbOpacitySettings: MandelbulbOpacitySettings`
...
const opacitySettings = useVisualStore((s) => s.mandelbulbOpacitySettings);
...
In `MaterialTabContent`, add conditional rendering for mandelbulb:

```tsx
const objectType = useGeometryStore((s) => s.objectType);
const isMandelbulb = objectType === 'mandelbulb';
...
{isMandelbulb && opacitySettings.mode === 'simpleAlpha' && (
  <Slider
    label="Face Opacity"
...
{isMandelbulb && opacitySettings.mode === 'layeredSurfaces' && (
  <>
...
{isMandelbulb && opacitySettings.mode === 'volumetricDensity' && (
  <>
